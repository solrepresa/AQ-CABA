...
setwd("/media/usuario/Elements SE/ARG_MERRA/raster_res") 
...
fecha <- as.data.frame(seq.Date(as.Date(ISOdate(2009,10,10)), as.Date(ISOdate(2017,10,01)), by = "day", tz = "GMT")) 
names(fecha) <- "fecha"
...
miss_data %>% filter( variable == "tavg1_2d_flx_Nx") %>%
...
falta_var <- falta %>% filter( variable == "tavg1_2d_rad_Nx")
...
names(missing_data_sds)
missing_data_sds <- missing_data_sds[, -c(7:11, 13,26)]
